...
async function main() {
  ...
  // create a queue
  await queueClient.createQueueIfNotExists("demo-queue", function (error) {
    if (error) console.log(`can not create queue ${error}`);
  });

  // create message queue
  await queueClient.createMessage(
    "demo-queue",
    "Hello world! #1",
  ...
  //console.log(mdata);
  // count = mdata.approximateMessageCount;

  // for (let i = 0; i < count; i++) {
  //   queueClient.getMessages(
  //     "demo-queue",
  //     function (error, results, response) {
  ...
  // clearing queue
  // await queueClient.clearMessages("demo-queue", function (error, response) {
  ...
  //   await queueClient.getQueueMetadata("demo-queue", function (error, mdata) {
  //     console.log(
  //       `message count after deleting: ${mdata.approximateMessageCount}`
  //     );
  //   });
}
  ...
  queueClient.createMessage(
    ...
    function (error, results, response) {
      ...
      queueClient.getQueueMetadata("demo-queue", function (error, mdata) {
        console.log(
          `message count after create: ${mdata.approximateMessageCount}`
        );
        ...
        queueClient.peekMessages(
          ...
          function (error, results, response) {
            ...
            queueClient.getQueueMetadata("demo-queue", function (error, mdata) {
              console.log(
                `message count after peek: ${mdata.approximateMessageCount}`
              );
                        ...
                        function (error, mdata) {
                          ...
                          queueClient.getMessages(
                            ...
                            function (error, results, response) {
                              this.message = results[0];
                              console.log(
                                `message text: ${this.message.messageText}`
                              );
                              queueClient.getQueueMetadata(
                                "demo-queue",
                                function (error, mdata) {
                                  console.log(
                                    `message count after get: ${mdata.approximateMessageCount}`
                                  );
