...
namespace QueuesGettingStarted
{
  ...
  public class Program : MessagingSamples.Sample
  {
    ...
    public async Task Run(string sendConnectionString, string getConnectionString)
    {
      this.receiveClient = new QueueClient(getConnectionString, BasicQueueName, ReceiveMode.PeekLock);
      this.InitializeReceiver();

      this.sendClient = new QueueClient(sendConnectionString, BasicQueueName);
    ...
    async Task SendMessagesAsync()
    {
      ...
      for (int i = 0; i < data.Length; i++)
      {
        ...
        message.UserProperties.Add("MyKey", $"Value{i}");
